...
parser = argparse.ArgumentParser(
    description="Runs maneuver, simulation and tests based on yaml file"
)
parser.add_argument("px4dir", help="PX4 src directory")
parser.add_argument(
    "maneuver_name",
    help="Name of the maneuver that we want to start. Also: Name of the yaml file that belongs to the maneuver",
)
...
if __name__ == "__main__":
    ...
    cmds = [
        "{0}/Tools/Maneuvers/build/maneuvers/{1}".format(
            px4_src_dir, args.maneuver_name
        ),
        "udp://:14540",
    ]
    ...
    cmds = [
        "gzserver",
        "{0}/Tools/sitl_gazebo/worlds/{1}.world".format(px4_src_dir, world),
    ]
    ...
    cmds = [
        "{0}/build/px4_sitl_default/bin/px4".format(px4_src_dir),
        "{0}/ROMFS/px4fmu_common".format(px4_src_dir),
        "-s",
        "etc/init.d-posix/{0}".format(start_script),
        "-d",
    ]
    env = os.environ.copy()
    ...
    for folder in log_folders:
        ...
        for log_name in log_files:
            ...
            print(
                "\n Run tests against the following log file: "
                + logdir
                + folder
                + "/"
                + log_name
            ...
            p_test = subprocess.Popen(
                [
                    ...
                    deselected_tests,
                    testfile,
                    "--filepath={0}".format(full_path_to_log),
                ]
            )
